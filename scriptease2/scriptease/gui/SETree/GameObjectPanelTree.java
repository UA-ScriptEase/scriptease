...
public class GameObjectPanelTree extends JPanel implements Observer{
	...
	private void addChildrenRoots(GameConversationNode parent){
		...
		Collection<Object> getChildren = treeModel.getTree().getSuccessors(parent);
		...
		for(Object a:getChildren){
			GameObjectPanelConversation newObjPanel = new GameObjectPanelConversation((GameConversationNode)a,CONSTANT_OFFSET*2);
	...
	private void expandTree(){
		ArrayList<Object> firstRowExpansion = (ArrayList<Object>) treeModel.getTree().getSuccessors(treeModel.getTree().getHead());
		int size = firstRowExpansion.size();
		for(int i=0; i < size ;i++){
			if(firstRowExpansion.get(i) instanceof String ){
				GameObjectLabel label = new GameObjectLabel(firstRowExpansion.get(i).toString());
				label.addObserver(this);
				this.add(label.getLabelPanel());
				gameObjectLabels.add(label);

				if(!firstRowExpansion.get(i).equals("dialogue")){
					Collection<Object> categoryExpansion = treeModel.getTree().getSuccessors(firstRowExpansion.get(i));
					for(Object getGameObject : categoryExpansion){	
						GameObjectPanel newObjPanel = new GameObjectPanel((GameObject) getGameObject,CONSTANT_OFFSET);
						this.addGameObjectPanelToTree(newObjPanel);

//						if( ((GameObject)getGameObject).getAttributes().size() > 0 ){
//							for(GameObject attributeObject : ((GameObject)getGameObject).getAttributes()){
//								GameObjectPanel attributeGameObjectPanel = new GameObjectPanel(attributeObject, CONSTANT_OFFSET+10);
//								this.addGameObjectPanelToTree(attributeGameObjectPanel);
//							}
//						}
					}
				}

				//The dialouge adding code goes here
				else if(firstRowExpansion.get(i).equals("dialogue")){
					Collection<Object> diagExpansion = treeModel.getTree().getSuccessors(firstRowExpansion.get(i));
					for(Object dialogueExpansionNode : diagExpansion){
						GameObjectPanel newObjPanel = new GameObjectPanel((GameConversation) dialogueExpansionNode,CONSTANT_OFFSET);
						this.gameObjectPanel.add(newObjPanel);
						this.add(newObjPanel.getGameObjectPane());

						Collection<Object> convExpansion = treeModel.getTree().getSuccessors(dialogueExpansionNode);
						for(Object gameConversation : convExpansion){	
							GameObjectPanelConversation newObjPanel2 = new GameObjectPanelConversation((GameConversationNode) gameConversation,CONSTANT_OFFSET*2);
	...
	private void reDrawTree(){
		...
		for(int i=0; i < size; i++){
			Collection<Object> srExp = treeModel.getTree().getSuccessors(gameObjectLabels.get(i).getLabel());
				...
				if(!gameObjectLabels.get(i).collapsed){
					...
					for(int j=0; j<srExp.size(); j++){
						if(z.get(j) instanceof GameObject){
							GameObjectPanel newObjPanel = new GameObjectPanel((GameObject) z.get(j), 10);
							this.gameObjectPanel.add(newObjPanel);
							this.add(newObjPanel.getGameObjectPane());
						}

						if(z.get(j) instanceof GameConversation){
							GameObjectPanel newObjPanel = new GameObjectPanel((GameConversation) z.get(j),CONSTANT_OFFSET);
							this.gameObjectPanel.add(newObjPanel);
							this.add(newObjPanel.getGameObjectPane());
							...
							Collection<Object> convExpansion = treeModel.getTree().getSuccessors(z.get(j));
							ArrayList<Object> bees3 = (ArrayList<Object>) convExpansion;
							for(int zl=0; zl < convExpansion.size(); zl++){

								GameObjectPanel newObjPanel2 = new GameObjectPanel((GameConversationNode) bees3.get(zl),CONSTANT_OFFSET*2);
								this.gameObjectPanel.add(newObjPanel2);
								this.add(newObjPanel2.getGameObjectPane());
								addChildrenRoots((GameConversationNode) bees3.get(zl));
							}
						}				
					}
				}
	...
	private class GameObjectLabel extends Observable implements MouseListener{
		...
		private void reDrawLabel(){
			...
			if(collapsed)
				labelPanel.add(new JLabel(convertToLabelViewFormat(),expandIcon, JLabel.CENTER));
			else
				labelPanel.add(new JLabel(convertToLabelViewFormat(),collapseIcon, JLabel.CENTER));
